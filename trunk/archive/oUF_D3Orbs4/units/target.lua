  ...
  local function d3o2_updatePlayerHealth(bar, unit, min, max)
  ...
  local function d3o2_updatePlayerPower(bar, unit, min, max)
    local d, d2
    local self = bar:GetParent()
    if max == 0 then
      d = 0
  ...
  --update health func
  local function d3o2_updateHealth(bar, unit, min, max)
    local self = bar:GetParent()
    local d = floor(min/max*100)
    local color
    local dead
  ...
  --update power func
  local function d3o2_updatePower(bar, unit, min, max)
    --if max == 0 then
    --  bar:SetAlpha(0)
    --else
    --  bar:SetAlpha(1)
  ...
  --postupdateauraicon
  local function d3o2_postUpdateAuraIcon(icons, unit, icon, index, offset)
  ...
  local function d3o2_postUpdateEnchantIcon(icons, unit, icon, index, offset)
    for i = 1, 2 do
      local icon = icons[i]
  ...
  --aura icon func
  local function d3o2_createAuraIcon(icons, button)
    local self = icons:GetParent()
  ...
  oUF.TagEvents["d3o2absmp"] = "UNIT_POWER UNIT_MAXPOWER"
  ...
  oUF.TagEvents["d3o2maxmp"] = "UNIT_POWER UNIT_MAXPOWER"
    ...
    if use_rbottombarstyler ~= 1 then
      ...
      if showplayerauras == 1 then
        ...
        --if self.Enchant then
          --self.Enchant.PostCreateIcon = d3o2_createAuraIcon
          --self.Enchant.PostUpdateIcon = d3o2_postUpdateEnchantIcon
        --end
        self.Buffs.PostCreateIcon = d3o2_createAuraIcon
        self.Debuffs.PostCreateIcon = d3o2_createAuraIcon
        self.Buffs.PostUpdateIcon = d3o2_postUpdateAuraIcon
        self.Debuffs.PostUpdateIcon = d3o2_postUpdateAuraIcon
      end
      
      self.Health.PostUpdate = d3o2_updatePlayerHealth
      self.Power.PostUpdate = d3o2_updatePlayerPower
  ...
  local function CreateTargetStyle(self, unit)
    ...
    self.Health.PostUpdate = d3o2_updateHealth
    self.Power.PostUpdate = d3o2_updatePower
    self.Buffs.PostCreateIcon = d3o2_createAuraIcon
    self.Debuffs.PostCreateIcon = d3o2_createAuraIcon
  ...
  if showpartyframes == 1 then
    ...
    local party = oUF:SpawnHeader("oUF_Party", nil, "raid,party,solo", "showParty", true, "showPlayer", true, "xOffset", 50, "point", "LEFT")
